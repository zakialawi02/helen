...
class GroundTruthOdometryProvider(OdometryProvider):
    r"""Ground truth odometry provider. Computes the relative transformation between a pair of `clifter_slam.RGBDImages`
    ...
    def provide(self, rgbdimages1: RGBDImages, rgbdimages2: RGBDImages) -> torch.Tensor:
        ...
        Args:
            rgbdimages1 (clifter_slam.RGBDImages): Object containing batch of reference poses of shape :math:`(B, 1, 4, 4)`
            rgbdimages2 (clifter_slam.RGBDImages): Object containing batch of destination poses of shape
                :math:`(B, 1, 4, 4)`
        Returns:
        ...
        if not isinstance(rgbdimages1, RGBDImages):
            raise TypeError(
                "Expected input 1 (rgbdimages1) to be of type clifter_slam.RGBDImages. Got {0}.".format(
                    type(rgbdimages1)
                )
            )
        if not isinstance(rgbdimages2, RGBDImages):
            raise TypeError(
                "Expected input 2 (rgbdimages2) to be of type clifter_slam.RGBDImages. Got {0}.".format(
                    type(rgbdimages2)
                )
            )
