...
class ICPSLAM(nn.Module):
    ...
    def forward(self, frames: RGBDImages):
        ...
        if not isinstance(frames, RGBDImages):
            raise TypeError(
                "Expected frames to be of type clifter_slam.RGBDImages. Got {0}.".format(
                    type(frames)
                )
            )
    ...
    def step(
        ...
        if not isinstance(live_frame, RGBDImages):
            raise TypeError(
                "Expected live_frame to be of type clifter_slam.RGBDImages. Got {0}.".format(
                    type(live_frame)
                )
            )
    ...
    def _localize(
        ...
        if not isinstance(pointclouds, Pointclouds):
            raise TypeError(
                "Expected pointclouds to be of type clifter_slam.Pointclouds. Got {0}.".format(
                    type(pointclouds)
                )
            )
        if not isinstance(live_frame, RGBDImages):
        ...
        if not isinstance(prev_frame, (RGBDImages, type(None))):
            raise TypeError(
                "Expected prev_frame to be of type clifter_slam.RGBDImages or None. Got {0}.".format(
                    type(prev_frame)
                )
            )
