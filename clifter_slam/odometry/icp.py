...
class ICPOdometryProvider(OdometryProvider):
    ...
    a pair of `clifter_slam.Pointclouds` objects using ICP (Iterative Closest Point). Uses LM (Levenberg-Marquardt) solver.
    """
    ...
    def provide(
        ...
        if not isinstance(maps_pointclouds, Pointclouds):
            raise TypeError(
                "Expected maps_pointclouds to be of type clifter_slam.Pointclouds. Got {0}.".format(
                    type(maps_pointclouds)
                )
            )
        if not isinstance(frames_pointclouds, Pointclouds):
            raise TypeError(
                "Expected frames_pointclouds to be of type clifter_slam.Pointclouds. Got {0}.".format(
